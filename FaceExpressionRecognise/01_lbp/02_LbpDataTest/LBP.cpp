...
int LBP::GetHopCount(int i)
{
	...
	{
		// 除2取余
		a[k] = i % 2;//a7为最高位   a7 a6 a5 a4 a3 a2 a1 a0
		i /= 2;
		--k;
	}
...
void LBP::ComputeLBPFeatureVector_256(const Mat &srcImage, Size cellSize, Mat &featureVector)//灰度不变LBP的特征向量计算
{																					//输入为原始图像，分块的个数，输出为特征向量
																					// 参数检查，内存分配
	CV_Assert(srcImage.depth() == CV_8U&&srcImage.channels() == 1);

	Mat LBPImage;
	ComputeLBPImage_256(srcImage, LBPImage);//srcImage->LBPImage，转化为灰度不变LBP 256维度
	...
	// 特征向量的个数
	int numberOfDimension = 256 * numberOfCell_X*numberOfCell_Y;//256*16*16
	featureVector.create(1, numberOfDimension, CV_32FC1);//创建一个256*16*16的向量用于存放
	featureVector.setTo(Scalar(0));

	// 计算LBP特征向量
	int stepOfCell = srcImage.cols;
	int pixelCount = cellSize.width*cellSize.height;//像素的个数
	...
	for (int y = 0; y <= numberOfCell_Y - 1; ++y)
	{
		for (int x = 0; x <= numberOfCell_X - 1; ++x)
		{
			...
			// 计算每个cell的LBP直方图
			Mat cell = LBPImage(Rect(x * widthOfCell, y * heightOfCell, widthOfCell, heightOfCell));//取刚才转化的LBP图像，进行分块
...
//srcImage:灰度图
//LBPImage:LBP图
void LBP::ComputeLBPImage_256(const Mat &srcImage, Mat &LBPImage)//输入为原始图像，输出为LBP灰度图像
{
	// 参数检查，内存分配
	CV_Assert(srcImage.depth() == CV_8U&&srcImage.channels() == 1);
...
void LBP::ComputeLBPFeatureVector_Uniform(const Mat &srcImage, Size cellSize, Mat &featureVector)//输入为原始图像，输出为等价模式的特征向量
{
	// 参数检查，内存分配
	CV_Assert(srcImage.depth() == CV_8U&&srcImage.channels() == 1);

	Mat LBPImage;
	ComputeLBPImage_Uniform(srcImage, LBPImage);//计算等价模式的LBP图像
	//cv::imshow("Result3", LBPImage);
	//cvWaitKey(0);
												// 计算cell个数
	int widthOfCell = cellSize.width;
	int heightOfCell = cellSize.height;
	...
	// 特征向量的个数
	int numberOfDimension = 58 * numberOfCell_X*numberOfCell_Y;//特征向量的个数为58*16*16
...
//计算旋转不变等价模式LBP的特征向量，输入为原始图像和分块大小，输出为特征向量
{
	// 参数检查，内存分配
	CV_Assert(srcImage.depth() == CV_8U&&srcImage.channels() == 1);

	Mat LBPImage;
	ComputeLBPImage_Rotation_Uniform(srcImage, LBPImage);//计算旋转不变等价模式的LBP图谱

														 // 计算cell个数
	int widthOfCell = cellSize.width;//cell宽度
	int heightOfCell = cellSize.height;//cell高度
	int numberOfCell_X = srcImage.cols / widthOfCell;// X方向cell的个数
	int numberOfCell_Y = srcImage.rows / heightOfCell;//Y方向cell的个数

													  // 特征向量的个数
	int numberOfDimension = 9 * numberOfCell_X*numberOfCell_Y;//特征向量个数为9*16*16
	featureVector.create(1, numberOfDimension, CV_32FC1);//创建列向量
	featureVector.setTo(Scalar(0));
